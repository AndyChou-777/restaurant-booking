...
import { Info, Search, UserCircle2, LogIn, Building2, User, LogOut } from "lucide-react"
          ...
          <NavigationMenuList>
            <NavigationMenuItem>
              ...
              <NavigationMenuContent className="absolute left-0 bg-gray-800 w-full max-w-[700px] p-4 box-border">
                <ul className="grid bg-gray-800 text-white w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
                  <ListItem href="/" title="關於我們" icon={<Info className="w-5 h-5 mb-2" />}>
                    旨在打造能夠滿足所有需求的線上餐廳預約平台，讓每個使用者都能輕鬆找到心儀的餐廳並完成預約。
                  </ListItem>
                  <ListItem href="/restaurant-reservation" title="搜索餐廳" icon={<Search className="w-5 h-5 mb-2" />}>
            ...
            {isLoggedIn? (
                ...
                <NavigationMenuContent className="absolute left-0 bg-gray-800 w-full max-w-[700px] p-4 box-border">
                  <ul className="grid bg-gray-800 text-white w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
                    {userRole === 'BUSINESS_USER' ? (
                      <ListItem href="/business/dashboard" title="企業會員管理" icon={<Building2 className="w-5 h-5 mb-2" />}>
                        管理您的預約狀態，增加顧客的來源。企業會員可以享受更多的功能，如設定特別優惠，查看預約統計數據，
                        以及增加顧客忠誠度的工具。
                      </ListItem>
                    ) : (
                      <ListItem href="/user/dashboard" title="一般會員管理" icon={<UserCircle2 className="w-5 h-5 mb-2" />}>
                    ...
                    <ListItem 
                      href="#" 
                      title="登出" 
                      icon={<LogOut className="w-5 h-5 mb-2" />}
                      onClick={(e) => {
                        e.preventDefault();
                        handleLogout();
                        showTemporaryAlert('登出成功', '用戶登出成功!', 'check', '/');
                      }}
                    >
              ...
              <NavigationMenuItem>
                ...
                <NavigationMenuContent className="absolute left-0 bg-gray-800 w-full max-w-[700px] p-4 box-border">
                  <ul className="grid bg-gray-800 text-white w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
                    <ListItem href="/login" title="登入" icon={<LogIn className="w-5 h-5 mb-2" />}>
            ...
            <NavigationMenuItem>
              ...
              <NavigationMenuContent className="absolute left-0 bg-gray-800 w-full max-w-[700px] p-4 box-border">
                <ul className="grid bg-gray-800 text-white w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
                  <ListItem href="/register/business" title="企業會員註冊" icon={<Building2 className="w-5 h-5 mb-2" />}>
                    企業會員註冊將讓您享有更多的管理功能與專屬服務。註冊後，您可以設定餐廳資訊，查看預約情況，
                    並進行顧客數據分析。
                  </ListItem>
                  <ListItem href="/register/user" title="一般會員註冊" icon={<User className="w-5 h-5 mb-2" />}>
    ...
    <li>
      <NavigationMenuLink asChild>
        <a
          ...
          {...props}
        >
          <div className="text-2x1 font-bold leading-none flex items-center">
